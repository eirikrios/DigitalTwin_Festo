...
export default function ActuatorDsnuStatus() {
  ...
  const calc = useMemo(() => {
    ...
    let light = 'green';
    if (pNum < P_MIN || pNum > P_MAX) light = 'red';
    else if (pertoMin || pertoMax)    light = 'yellow';
  ...
  async function handleStart() {
    setStarted(false);
    const dsnu = { pressao: Number(pressao) || 0, carga: Number(carga) || 0 };
    const dsbc = { pressao: 0, carga: 0 };
  ...
  return (
    <div className="status-right">
      ...
      <p className="status-line" style={{ marginTop: 6 }}>
        Limites DSNU: {P_MIN}–{P_MAX} bar | Stroke: {STROKE} mm | v<sub>máx</sub>≈ {VMAX} mm/s
      </p>
